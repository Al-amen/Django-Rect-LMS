...
import { PAYPAL_CLIENT_ID } from "../../utils/contants";
...
function Checkout() {
  ...
  const fetchOrder = async () => {
    try {
      await apiInstance.get(`order/checkout/${pram.order_oid}/`).then((res) => {
        setOrder(res.data);
      });
    } catch (error) {
      console.log(error);
  ...
  const applyCoupon = async () => {
    ...
    formdata.append("order_oid", order?.oid); // ✅ Fixed key name
